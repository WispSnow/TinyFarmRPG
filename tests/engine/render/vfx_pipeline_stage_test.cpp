...
TEST(VfxPipelineStageTest, PassTypeInsertsVfxBetweenBloomAndUi) {
    ...
    const std::size_t pos_vfx = content.find("Vfx,");
    const std::size_t pos_ui = content.find("UI,");

    ASSERT_NE(pos_bloom, std::string::npos);
    ASSERT_NE(pos_vfx, std::string::npos);
    ASSERT_NE(pos_ui, std::string::npos);
    EXPECT_LT(pos_bloom, pos_vfx);
    EXPECT_LT(pos_vfx, pos_ui);
}

TEST(VfxPipelineStageTest, PresentRunsVfxAfterCompositeBeforeUi) {
    ...
    ASSERT_NE(pos_vfx, std::string::npos);
    ASSERT_NE(pos_ui, std::string::npos);
    EXPECT_LT(pos_composite, pos_vfx);
    EXPECT_LT(pos_vfx, pos_ui);
}
