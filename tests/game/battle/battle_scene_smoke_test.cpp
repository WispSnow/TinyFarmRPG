...
TEST(BattleSceneSmokeTest, UsesTypedModelAndSceneLevelMenuInput) {
    ...
    EXPECT_NE(view_model_builder_source.find("battlePartyPortraitDecoratorForUnit"), std::string::npos);
    EXPECT_NE(view_model_builder_source.find("image(battle-party-portrait-player)"), std::string::npos);
...
TEST(BattleSceneSmokeTest, RendersSideViewSpritesOverOpaqueBattlefield) {
    ...
    EXPECT_NE(source.find("scale = actor->battle_visual_.sprite_scale_"), std::string::npos);
    EXPECT_NE(source.find("scale = enemy->battle_visual_.sprite_scale_"), std::string::npos);
...
TEST(BattleSceneSmokeTest, RcssDefinesStage5BattleMenuStates) {
    ...
    EXPECT_NE(rcss.find("battle-party-portrait-player: 8px 8px 48px 48px;"), std::string::npos);
    EXPECT_NE(rcss.find("battle-party-portrait-lyria: 8px 8px 48px 48px;"), std::string::npos);
    EXPECT_NE(rcss.find("battle-party-portrait-tori: 8px 8px 48px 48px;"), std::string::npos);
    const std::string portrait_block = snippetFrom(rcss, ".battle-portrait {");
    ASSERT_FALSE(portrait_block.empty());
    EXPECT_NE(portrait_block.find("width: 48dp;"), std::string::npos);
    EXPECT_NE(portrait_block.find("height: 48dp;"), std::string::npos);
    ...
    const std::string party_stats_block = snippetFrom(rcss, ".battle-party-stats {");
    ASSERT_FALSE(party_stats_block.empty());
    EXPECT_NE(party_stats_block.find("margin-top: 5dp;"), std::string::npos);
