...
这个对比就是本讲的核心：**战斗速度只加速了"表现"，没碰"逻辑"**。如果它是粗暴地缩放 `dt`，那连随机数、状态计时都会跟着变，伤害数字就可能不同了。它没有——因为速度只除在表现时间轴上：导演 pose 的各段时长、VFX/SFX/HP reveal marker 的触发时刻、飘字的 impact 延迟都会一起缩放，但领域结算不动。本讲就讲清楚：领域那个 `BattleActionResult` 是怎么被一层层翻译成画面的，而这层翻译又怎么做到"快慢随你调、真相纹丝不动"。
...
这是自测题 2 的答案：**速度只缩放表现时间轴里的绝对时刻与时长。** 为什么不直接缩 `dt`？因为 `dt` 是整个战斗场景的逻辑时钟——缩它会一并加速 HP 条平滑、飘字生命周期、甚至将来任何吃 `dt` 的逻辑，难以预测，也可能影响确定性。而玩家想要的只是"动作演得快点"，**伤害、命中、胜负这些真相必须和速度无关**。把速度限制在"表现事件什么时候发生"这一个明确的旋钮上，既满足需求，又守住了那道墙。
...
下一讲 **L20 战斗结算、奖励与探索态写回**：`BattleRewardResolver` 怎么聚合金币/掉落/经验，`BattleEndedEvent` 回到 `GameScene` 后的**固定写回顺序**（物品消耗 → 金币 / 掉落 → 经验 → 任务），失败/逃跑/胜利三类 outcome 的不同处理，以及战斗内物品副本如何与真实背包合并——L17 那条"运行时副本、结束按差额集中写回"的主线，在这一讲收口。Stage V 战斗闭环就此完成。下讲见。
