...
打开 [`scripts/npcs/greeter.lua`](../../scripts/npcs/greeter.lua)——一个最简单的 NPC，34 行：
...
| 在战斗胜利后播一段剧情 | **Lua**（`tf.battle.on_battle_ended` 监听） |
...
打开 [`tinyfarm_script_module.cpp`](../../src/game/script/tinyfarm_script_module.cpp)（详深 L07 讲），里面 `tf` 顶层下挂了 12 个子命名空间：
...
| `tf.battle` | 启动战斗、监听战斗事件 | `tf.battle.start("troop.goblin_camp")` |
| `tf.map` | 传送、查询地图 | `tf.map.warp("home_interior", 5, 3)` |
| `tf.state` | 持久化脚本状态 | `tf.state.set("flag.x", true)` |
| `tf.command` | 发玩家命令（背包 / 宝箱） | `tf.command.add_item("potato_seed", 3)` |
| `tf.time` | 查询游戏时间 | `tf.time.is_night()` |
...
| `tf.player` | 玩家自身查询 | `tf.player.exists()` / `tf.player.set_gold(300)` |
...
打开 [`scripts/bootstrap.lua`](../../scripts/bootstrap.lua)——开头一段注释直接点明：

```lua
-- GameRuntimeAssembler loads this file every time GameScene is initialized,
-- including after loading a save. Keep top-level work idempotent;
-- persistent story state must go through tf.state.
```

意思是：**`bootstrap.lua` 在每次 `GameScene` 初始化时都会被重新加载**——包括玩家从主菜单开始新游戏、从存档读档、退到标题再回来。**整个 Lua VM 会被销毁并重建**，所有 module-local 变量回到初始值。
...
**重建 VM** 让脚本与存档状态在每次进入 `GameScene` 时**强制同步**——只要你的剧情状态全部走 `tf.state`，重建后 Lua 看到的世界就是存档里的世界。

### 6. 关键规约 ②：持久剧情状态走 `tf.state`

`tf.state` 是 Lua 持久化的唯一通道。打开 [`scripts/lib/state.lua`](../../scripts/lib/state.lua)——19 个 export 函数全部代理给 `tf.state.*`：
...
| 典型场景 | 任意持久状态：金币计数、剧情阶段、玩家选择 | "一次性 hook"：宝箱、首次提示、剧情触发 |
...
这段做了什么——"如果之前没给过 300 金币，就给"。**注意顺序**：先调 `set_gold`，**成功后**再 `state.set`——这样如果 `set_gold` 失败（如玩家实体还没生成），下次重新加载脚本时还会重试，保证最终一致性。这种"业务操作成功后再 mark flag"是 `state` 比 `once.run` 更适合的场景。
...
| `event.lua` | `on_day_changed` / `on_battle_ended` 等事件注册的语法糖 | L08 |
...
| 2 | [`scripts/bootstrap.lua`](../../scripts/bootstrap.lua) | 组合根本身。注意开头注释说明的"幂等"约定 |
...
| 5 | [`scripts/npcs/greeter.lua`](../../scripts/npcs/greeter.lua)（30 行） | 最简 NPC 脚本模板 |
...
| 顺序 | 文件 | 你会看到什么 |
| :---: | --- | --- |
| 1 | [`src/game/script/script_state.h`](../../src/game/script/script_state.h) | `ScriptStateStore`——`tf.state` 背后的 C++ 实现，62 行，本质是个 typed variant map |
| 2 | [`src/game/script/tinyfarm_script_module.cpp`](../../src/game/script/tinyfarm_script_module.cpp)（搜 `// ── tf.`） | 12 个子命名空间的注册——只看分块标记，**详深留 L07** |

---

## ❓ 自测问题

1. **幂等性**：把"给玩家 300 金"的逻辑放在 `bootstrap.lua` 顶层，**不**用 `tf.state` flag——读 5 次存档会发生什么？
...
1. 打开 [`scripts/lib/state.lua`](../../scripts/lib/state.lua)（19 行）和 [`scripts/lib/once.lua`](../../scripts/lib/once.lua)（30 行）。
2. 列出至少 **3 个**两者在"调用方式"上的差异。
3. 找到 `once.run` 里 `state.set(key, true)` **在 `fn()` 之前** 的那一行——用一句话解释**为什么是这个顺序而不是反过来**。
4. 在 [`scripts/maps/home_exterior.lua`](../../scripts/maps/home_exterior.lua) 找一处 `once.run` 的真实用法，跟踪它的 `key` 在 `bootstrap.lua` 重新加载时如何被"记住"。
...
- `tf.*` 顶层提供 12 个子命名空间覆盖 dialogue / quest / party / shop / battle / map / state / command / time / entity / event / i18n。
- **两条核心规约**：
  - **脚本顶层幂等**——每次 `GameScene` 初始化都会重建 Lua VM，module-local 变量不可作为持久存储。
