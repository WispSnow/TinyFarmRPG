...
flowchart TD
    ...
    PARTY -.->|"capture / apply"| SAVE["存档 party_state"]
```
...
struct PartyComponent {
    ...
    std::size_t max_active_members_{4};                               // 参战上限
};
```

队伍就是**两个字符串列表**：`recruited`（招过的所有人）和 `active`（这次带谁上场）。**没有"队友实体"这种东西**——Lyria 入队后，不是在玩家身边生成一个 Lyria 实体跟着跑，而是 `"actor.lyria"` 这个字符串被 push 进了 `recruited_actor_ids_`。
...
注意 `recruited`/`active` 都默认包含 `"actor.player"`（[`party_ids.h`](../../src/game/defs/party_ids.h) 里 `kDefaultPlayerActorId = "actor.player"`），而且 `PartyRecruitmentSystem` 每次写入前都会 `ensureDefaultPartyActor`——**玩家永远在队里**，这是不变量，不靠数据正确性保证。
...
flowchart LR
    ...
    ROSTER -->|"存档存的就是<br/>这串 id"| SAVE["party_state"]
```
...
回到 `max_active_members_{4}`。它是 `PartyComponent` 的一个**成员字段**，默认 4——**不是**散落在逻辑里的 `#define` 或魔法数字。这个区别很重要：

- 它是**运行时数据**：每个 `PartyComponent` 实例带自己的上限，可以在创建时按难度/剧情设不同值。
- 它**进存档**（`party_state`，见文档存档关系表），意味着"队伍上限"是可以随存档变化的状态，而非编译期常量。
- 系统逻辑里所有"满没满"的判断都读这个字段（`party.active_actor_ids_.size() < party.max_active_members_`），改一处即可全局生效。

当前它没有从 JSON 配置加载（默认值写在结构体里），但**形态上它是数据而非硬编码**——想做成配置驱动，只需在初始化时从某处赋值，无需改任何判断逻辑。

> **回到自测题 4**：4 是 `PartyComponent::max_active_members_` 的默认值，一个进存档的运行时字段，不是写死的常量。判断满员的逻辑读字段，所以扩展成配置驱动几乎零成本。
...
| 3 | [`src/game/component/party_component.h`](../../src/game/component/party_component.h) | 队伍真相只是两个 id 列表 + 一个上限 |
| 4 | [L08 脚本事件桥与 Tiled 接入](08-脚本事件桥与Tiled接入.md) | 回看 `actor_id` 作为稳定脚本身份的来历 |
...
5. **上限**：要做一个"剧情前期队伍上限 2、后期解锁到 4"的需求，你会改哪里？需要动判断逻辑吗？
...
- **队伍上限是数据**：`max_active_members_` 是进存档的运行时字段（默认 4），判断满员读字段，改成配置驱动几乎零成本。
