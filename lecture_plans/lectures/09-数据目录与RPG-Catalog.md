...
flowchart TD
    ...
    JSON["classes / actors / skills / states<br/>equipment / enemies / troops .json"] --> LOADER
    LOADER --> PARSE["逐文件 load*<br/>解析 + 算 id_hash + 单文件校验"]
    PARSE --> VALIDATE["validateReferences<br/>跨文件引用校验 (含 ItemCatalog)"]
    VALIDATE -- "失败" --> ABORT["spdlog::error<br/>内容加载失败，游戏起不来"]
    VALIDATE -- "成功" --> SERVICES["GameRuntimeServices.rpg_catalog<br/>只读查询结构"]
    SERVICES --> BATTLE["战斗 / 队伍 / 装备 system"]
    SERVICES --> LUA["Lua tf.* 按 id 选择与触发"]
    SERVICES --> UI["角色面板 / 战斗 UI"]
```

记住一句话：**JSON 是真相 → catalog 加载并在启动时一次性校验所有引用 → 校验过了才交给任何系统使用**。这是一道"fail-fast 闸门"——断引用永远在启动时暴露，而不是等你打到那场战斗才崩。
...
- **schema_version 必须非零**：[`rpg_catalog.cpp:34`](../../src/game/data/rpg_catalog.cpp) 里 `schema_version == 0` 直接拒绝加载——防止加载到一个格式未知的旧档。
- **`files` 必须 7 个全齐**：[`rpg_catalog_loader.cpp:72`](../../src/game/runtime/rpg_catalog_loader.cpp) 里任一映射缺失就整体失败。这是"宁可起不来，也不要带着残缺数据跑"。
...
这是本讲最该看懂的一段代码。打开 [`rpg_catalog.cpp` 的 `validateReferences`](../../src/game/data/rpg_catalog.cpp)（117–237 行），它在**所有 7 个文件加载完之后**，把整张引用网走一遍：
...
**它在什么时候跑？** 看调用链 [`rpg_catalog_loader.cpp:107`](../../src/game/runtime/rpg_catalog_loader.cpp)：
...
再往上，[`content_catalog_loader.cpp`](../../src/game/runtime/content_catalog_loader.cpp) 的 `ensureRpgCatalog` 会 `spdlog::error` 打出来并返回 false——**游戏内容加载阶段直接失败**。
...
而 catalog 内部的存储，**全部用 hash 当 key**（[`rpg_catalog.h:54-60`](../../src/game/data/rpg_catalog.h)）：

```cpp
std::unordered_map<entt::id_type, SkillData> skills_{};   // key 是 id_hash，不是字符串
```

哈希怎么算的？就一行（[`rpg_catalog.cpp:23`](../../src/game/data/rpg_catalog.cpp)）：
...
这也解释了 `validateReferences` 里那段对 equipment 的严格校验（[`rpg_catalog.cpp:192-211`](../../src/game/data/rpg_catalog.cpp)）：装备引用的 item 必须 `category == Equipment`、`stack_limit == 1`、且**没有** `on_use`/`battle_use`——保证两个 catalog 对同一 id 的理解不打架。

**这种跨 catalog 引用，决定了加载顺序不能乱。** 看 [`content_catalog_loader.cpp`](../../src/game/runtime/content_catalog_loader.cpp) 的 `ensure*` 顺序：

```mermaid
flowchart LR
    BP["Blueprint"] --> ITEM["ItemCatalog"]
    ITEM --> APP["Appearance"]
    APP --> VFX["VfxCatalog"]
...
- **可校验**：[`AudioCueCatalog::validateReferences`](../../src/game/data/audio_cue_catalog.h:37) 会拿 `AssetRegistry` 检查 cue 引用的 `music_id` 是否真的注册了资源——写错音乐 id 同样 fail-fast，而不是进了场景一片寂静才发现。
...
| 4 | [`tools/rpg_importer/README.md`](../../tools/rpg_importer/README.md) | 离线导入工具：RPG Maker → 项目格式 + 语义 id 别名 |
...
| 6 | [`src/game/data/audio_cue_catalog.h`](../../src/game/data/audio_cue_catalog.h) | 一个"麻雀虽小五脏俱全"的 catalog：load + validate + find + 默认 cue |
| 7 | [`tests/game/rpg_catalog_test.cpp`](../../tests/game/rpg_catalog_test.cpp) | 一排 `ValidateFailsOn...` 反向用例——校验失败时错误信息长什么样 |
...
4. **观察测试视角**：跑 `rpg_catalog_test`（`ProjectAssetsExposeSlimeTroopForMapEncounter` 这个用例会加载**真实** `assets/data/rpg` 资源），它会在断引用时直接失败——这就是 CI 能拦住你"提交一个断引用"的原因。

**进阶**：

- 阅读 [`tools/rpg_importer/README.md`](../../tools/rpg_importer/README.md)。这个离线工具从 `for_agent/ref/data`（RPG Maker 原始 JSON）导入并生成 `validation_report.json` 做**导入期**的基础引用校验——和运行时的 `validateReferences` 是**两道不同的关卡**（导入期 vs 启动期）。想想：为什么离线导入做一遍校验、运行时还要再做一遍？
