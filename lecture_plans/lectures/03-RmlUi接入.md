# 第3课 从自研 UIManager 到 RmlUi

上节课我们把"写入规则"的边界收紧到了 `src/game/domain/`。这节课反过来看 UI 这一侧——当玩家打开背包、点开商店、走进对话，**屏幕上能呈现的一切都来自哪里？**
...
这节课只回答两个核心问题：**为什么要换、接入点在哪里**。RmlUi 自身的 RML/RCSS 语法**完全不在主线展开**——子教程已经把它讲透了，这节课只讲"项目层面怎么接"。
...
如果你写过任何一点 HTML/CSS，这就是你熟悉的味道——**声明式标记 + 外链样式表**。这正是 RmlUi 被选中的根本原因：UI 应该用标记语言描述，而不是塞在 C++ 里一行行 `new UIPanel(...)`。

想在运行中看到它，可以打开游戏后按 **F5**，在 `Engine Debug Panels` 里勾选 **RmlUi**。这个面板会扫描 `ui/rmlui/` 下的 `.rml` 文件。选择 `ui/rmlui/learn/learn_hello.rml` 后点击 **Load**，就能把学习文档挂到当前 RmlUi runtime。修改 `learn_hello.rcss` 后，再在 `Debug Documents` 列表里点 **Reload**，样式即刻刷新。

这里的 Reload 是**调试文档**入口：runtime 先加载新文档，成功后替换旧文档，失败时保留旧文档。正式生产 UI 仍由各个 Scene / HUD 的 `RmlDocumentController` 管理生命周期——本课不要求你给生产菜单做热重载。
...
上一期 [part-17](../ref/OpenGL与迷你农场/17-UI%20框架基础.md) 与 [part-18](../ref/OpenGL与迷你农场/18-UI%20布局与预设.md) 讲过自研方案：
...
**这套方案在 TinyFarm 里完全够用**——按钮、面板、对话气泡、几个数值显示，需求就这么多。
...
如果继续用自研方案，会**很快撞上三个瓶颈**：

- **静态描述能力不足**。每个新菜单都要写一堆 `new UIPanel(...)`、`new UIButton(...)`、手算 padding——程序员被卷进了"美工 + 排版工"的工作。
- **状态绑定靠手抄**。当 HP 从 100 掉到 80，要从 C++ 找到对应 UIElement → `setText("80")` → 通知重绘——每个数值字段都要手动走一遍。漏掉一个，UI 上的数字就不动。
- **样式无法独立迭代**。换个按钮配色、改个间距，要重编 C++。美术和策划完全无法独立调参。
...
RmlUi 本质上是一个**类浏览器渲染器**——把 HTML/CSS 子集编译进了引擎里。它在三个维度上根本性地改变了 UI 开发方式：
...
- **RmlUi Debugger**。内置 inspector，项目默认启用。运行中按 F4 可切换可见性，点节点看样式、看盒模型。

> **结论**：换 RmlUi 不是因为自研 UIManager"不好"，而是 **JRPG 玩法的 UI 复杂度已经超过了自研方案的甜区**。继续往自研框架里加 grid、tab、滑块、列表绑定，最后会做出一个"功能比 RmlUi 少、稳定性比 RmlUi 差"的山寨版浏览器引擎。

### 3. 接入层四件套

打开 [`src/engine/ui/rmlui/`](../../src/engine/ui/rmlui)，会看到一组接入文件。**只需要记住四个角色的职责分工**：
...
RmlUi 没有自己的字体资源管理器——它需要项目提供入口。TinyFarmRPG 把这件事接到了和 TinyFarm 一样的字体路径上：
...
图像加载要说得更精确一点。`RenderInterface_GL3_STB::LoadTexture()` 内部使用 stb_image 解码：

```cpp
pixels = stbi_load_from_memory(file_buffer.data(), ..., STBI_rgb_alpha);
```

`stb_image.h` 也是 game 层纹理加载的主路径，所以这里共享的是**解码库与像素格式约定**，不是 `ResourceManager / TextureManager` 的缓存。普通游戏 sprite 走 `ResourceManager::loadTexture()`，RmlUi `<img>` 则由 RmlUi 的 `FileInterface` 读文件，再由 `RenderInterface_GL3_STB` 生成 RmlUi 自己的 texture handle。一句话：没有引入第二套图片解码库，但同一张 PNG 如果同时被 sprite 和 RmlUi 使用，可能各自拥有自己的 GPU 纹理。

> **特殊情况**：动态生成的图像（如头像预览、地图预览）走 `RmlGeneratedImageRegistry` 注册一个 `generated://...` source，例如 `generated://player-portrait/<scene_id>` 或 `generated://map-preview/<map_name>`。`LoadTexture` 看到这个前缀就跳过文件 IO，直接从 registry 取 CPU-side `DecodedImage`。细节留到 [分层角色外观与头像](14-分层角色外观与头像.md)。
...
任何具体场景 RML（如 `shop_menu.rml`）只用 `<link>` 引入 theme，再写自己**独有的布局差异**。这样换皮、改主色、调整 modal 阴影只动 theme 一处，所有场景自动生效。
...
所以这节课看到 `data-event-click` 时，先把它理解成**鼠标点击与 RML data event 的边界**。键盘/手柄菜单导航的上下文、缓冲与路由，留到 [输入上下文与菜单导航](05-输入上下文与菜单导航.md) 专门讲。
...
| 2 | [`docs/engine/ui_framework.md`](../../docs/engine/ui_framework.md) | **本节课核心阅读材料**——RmlUiRuntime / DocumentController / 文档目录组织约定 |
...
| 5 | RmlUi 子教程 | **前 6 节是前置必修**（文档结构、盒模型、布局、样式、事件、数据绑定）。后 9 节穿插推荐，主线后续课程会点名 |

> ⚠️ **如果你跳过了 RmlUi 子教程前 6 节直接进入这节课，会在看 `.rml/.rcss` 时反复卡住语法**。强烈建议先把这 6 节走完再继续。
...
1. 复制 [`learn_hello.rml`](../../ui/rmlui/learn/learn_hello.rml) + [`learn_hello.rcss`](../../ui/rmlui/learn/learn_hello.rcss)，重命名为 `my_first.rml` / `my_first.rcss`。
...
6. **不要写 C++ 事件绑定**——这节课只确认文档能渲染。点击按钮无反应是正常的，事件绑定属于 RmlUi 子教程的内容。
...
- 当前输入口径是鼠标优先：`data-event-click` 是本节课重点，完整键盘/手柄菜单导航留到 [输入上下文与菜单导航](05-输入上下文与菜单导航.md)。

---

## 下节课预告

RmlUi 怎么进来讲完了。下节课 **[HUD 与覆盖式场景](04-HUD与覆盖式场景.md)** 讲它怎么被组织起来——`GameScene` 入场时挂多份常驻 HUD（hotbar / clock / dialogue_box / fade），玩家按 ESC 又叠加 Pause / Inventory / Shop / Battle 等覆盖式 Scene。**显隐切换 vs Scene push/pop** 在什么时候用哪种？这是把 RmlUi 真正用起来的关键决策。
