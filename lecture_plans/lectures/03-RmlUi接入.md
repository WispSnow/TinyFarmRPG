...
4. RmlUi 怎么共享上一套的字体与图片资源系统，而不重复一套加载？
...
- **热重载**：改 RML/RCSS 不用重编 C++，运行时按"刷新当前文档"重新加载。
- **RmlUi Debugger**：内置调试器，可以像 Chrome DevTools 那样点节点看样式 / 看盒模型。
...
| 角色 | 文件 | 职责 |
| --- | --- | --- |
| **`RmlUiRuntime`** | [`rml_ui_runtime.*`](../../src/engine/ui/rmlui/rml_ui_runtime.h) | 全局唯一。持有 `Rml::Context`、转发 SDL 事件、按 `owner_scene_id` 加载 / 卸载文档、每帧调用 `update()` |
...
图像加载更直接。`RenderInterface_GL3_STB::LoadTexture()` 内部就是用 [stb_image](../../external/) 解码：

```cpp
pixels = stbi_load_from_memory(file_buffer.data(), ..., STBI_rgb_alpha);
```

`stb_image.h` 也是 TinyFarm 项目纹理加载的主路径。这意味着 RmlUi 用的图像解码栈和 game 层纹理资源**完全一致**——同一张 PNG，在背景里被 Sprite 用、在 UI 里被 `<img>` 用，没有第二条解码路径。

> **特殊情况**：动态生成的图像（如战斗里实时合成的角色头像）走 `RmlGeneratedImageRegistry` 注册一个特殊 source 字符串（形如 `gen:portrait_001`），`LoadTexture` 看到这个前缀就跳过文件 IO，直接从 registry 取已解码像素。详深留 L14。
...
| 4 | **RmlUi 子教程入口**：[`learn/lectures/rmlui/syllabus.md`](../../learn/lectures/rmlui/syllabus.md) | **前置必修 L01–L06**：文档结构、盒模型、布局、样式、事件、数据绑定。L07–L15 穿插推荐，主线后续讲次会点名 |
...
| 4 | [`src/engine/core/game_app.cpp`](../../src/engine/core/game_app.cpp)（`initRmlUi`，约第 420 行） | 字体注册、viewport 同步、render hook 注入的"接入主函数" |
| 5 | [`ui/rmlui/theme/base.rcss`](../../ui/rmlui/theme/base.rcss) | 全局基线样式表，看主题层做什么 |
...
3. **资源共享**：如果项目要再加一种 UI 字体，怎么注册才能同时让 ImGui 调试面板与 RmlUi 生产 UI 都能用？
...
3. 在 RCSS 里把背景色 / 字体色随便改一下，确认 reload 后视觉真的变了。
4. 找到加载 learn 文档的位置（提示：grep `learn_hello.rml`），观察项目用什么调用路径触发 `loadDocument`。
5. **不要写 C++ 事件绑定**——这一讲只确认文档能渲染。点击按钮无反应是正常的，事件绑定属于 RmlUi 子教程 L05 的内容。
...
- 资源共享：字体走 `loadFontFace`（与 ImGui 共用 TTF），图像走 stb_image（与 game 层纹理共用解码栈）。
