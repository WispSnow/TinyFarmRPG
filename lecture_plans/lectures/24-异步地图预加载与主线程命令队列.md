# 第24节课 · 异步地图预加载与主线程命令队列

上一套教程里，切地图是**同步**的：读文件、解析 JSON、解码图片、上传 GPU——全挤在切图那一帧里干完。地图一大、贴图一多，你就能肉眼看到明显的冻帧卡顿。这节课讲项目怎么把这件事**异步化**，让切图延迟接近于零。
...
关键观察两点：① `worker done` 和 `commit done` 通常**不在同一帧**——worker 把上传命令投进队列后，要等主线程**下一次 drain** 才执行；② `preprocess_ms` + `decode_ms` 这两段重活，全发生在 worker 线程，主线程那一帧几乎没被它们拖住。这个"跨线程、跨帧的接力"就是这节课的主角。
...
这套队列是**通用基础设施**，不只服务于切图——[存档课](21-存档系统与Schema迁移.md)的后台存档同样用它把"写盘后的收尾"调度回主线程（[`save_service.cpp`](../../src/game/save/save_service.cpp) 也取 `getMainThreadCommandQueue()`）。"把活儿调度回主线程"是个反复出现的模式。
...
注意 `async_preload_tasks_` 这张任务表（`unordered_map`）**本身不加锁**。凭什么线程安全（问题 3）？因为所有公共 API（`schedule` / `getTaskState` / `markAppliedIfReady` / `clearTasks` / `setLoadingSettings`）开头都调 `ensureOwnerThread`——**只有 owner（主）线程能增删改这张表**；worker 线程从不碰它，只通过 `shared_ptr<AsyncPreloadTaskState>` 读写那两个 atomic 字段。
...
`~AsyncPreloadPipeline()` 调 `clearTasksImpl`，安全点是刚性的（问题 4）：
...
| 多线程子教程 [`README`](../../docs/tutorial/multi-thread/README.md) + 03 有界队列 / 05 主线程命令队列 / 06 异步管线 / 07 线程安全实践 | 基础原理（本节课用到的队列、命令队列、管线、owner 约定都在这里展开） |
...
| 文件 | 看什么 |
| --- | --- |
| [`src/game/world/async_preload_pipeline.h`](../../src/game/world/async_preload_pipeline.h) / [`.cpp`](../../src/game/world/async_preload_pipeline.cpp) | **本节课主角**：状态机、generation、owner 契约、worker lambda、析构顺序 |
...
- **`MainThreadCommandQueue` 把 GPU 活儿预约回主线程**：owner 限定 drain、`DrainPolicy` 限额（数量+时间预算）、入队背压；是通用设施，[存档课](21-存档系统与Schema迁移.md)后台存档也用它。
...
本节课是项目里第一个并发模式——异步预加载。下节课是第二个。

下节课讲 SystemScheduler 与并行岛：上一套教程里 gameplay 系统是"固定顺序挨个 update"的，下节课把它升级成**可观察、可裁剪、可并行**的调度器——用每个系统声明的"资源读写"自动推导出哪些系统能并行成一个 wave，哪些必须排队；顺便收口我们在 [HUD 课](04-HUD与覆盖式场景.md)、[战斗过渡课](15-探索与战斗的过渡.md)反复提到却一直没展开的 `GameMode`（Exploration / Battle / PauseOverlay / Cutscene 怎么影响调度），还能用 `tools/scheduler_dot_dump` 把调度图导成 DOT 可视化排查。下节课见。
