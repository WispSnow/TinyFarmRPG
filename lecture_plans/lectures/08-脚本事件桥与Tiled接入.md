...
1. 一个 Tiled 对象同时配了 `actor_id`（脚本化）和默认 C++ 交互组件（如 `DialogueComponent`）时，谁优先响应？为什么？
2. 区域触发器（`script_zone`）的 "一次性" 语义是写在 C++ 系统里还是 Lua 里？哪种更合理？
3. `tf.dialogue.choice(...)` 弹出选项后，玩家选了第 2 个，结果怎么找到原 Lua 回调？
...
打开 [`scripts/npcs/greeter.lua`](../../scripts/npcs/greeter.lua)（30 行）。重点看 `interact` 回调里 `evt` 这个 table 上有什么字段——这些字段就是 C++ 通过 `ScriptEventBridge` 递给 Lua 的所有信息：
...
**关键观察**：所有"该不该处理这次交互"的判断**全都基于稳定身份字段**（`actor_id` / `script_event` / `script_kind`），从来不依赖显示名 / 槽位 ID 之类的易变字段。
...
```mermaid
flowchart TD
    TILED["Tiled 地图<br/>actor_id / scripted_interaction / script_event / zone_id"] --> LOAD["EntityBuilder<br/>解析属性 → 挂 component"]
...
> **回到自测题 1**：如果你想让"Tiled NPC 同时走 C++ 任务系统 + Lua 多说一句"——**不要**给它加 `scripted_interaction=true`。让默认 C++ 任务系统处理任务流程，Lua 在 `interact` 回调里**只追加**额外对白（脚本里检查 `evt.dialogue_handled` 已被 C++ 设置过，再决定是否补对话）。

### 2. Tiled 属性约定：四个 "magic" 字段

打开 [`src/game/loader/tiled_conventions.h`](../../src/game/loader/tiled_conventions.h)，看到一组 `OBJECT_PROP_*` 常量。Lua 内容最常依赖的 4 个：
...
**为什么字段这么多？** 每个字段对应**一个独立的交互形态**。同一个 NPC 可以同时是"任务发布者 + 招募候选 + 脚本化对白"——三种身份各自挂自己的 component，由各自系统响应。
...
void ScriptEventBridge::onInteract(const InteractCommand& event) {
    ...
    if (const auto* identity = registry_.try_get<ActorIdentityComponent>(event.target)) {
        ...
        // ... actor_id_hash / blueprint_id
    } else {
        payload["target_actor_id"] = sol::lua_nil;
    }

    // 从 NameComponent 拷贝显示名
    // 从 ScriptTriggerComponent 拷贝 script_event / script_once_key
...
- **稳定身份优先**：payload 里既有 `target_actor_id`（字符串）也有 `target_actor_id_hash`（hashed_string，性能更高），脚本两者皆可比较。
...
`tf.dialogue.choice(prompt, options, callback)` 是 Lua 推选项的主入口。完整链路：
...
sequenceDiagram
    ...
    L->>API: tf.dialogue.choice(prompt, options, cb)
    API->>API: 生成唯一 request_id
    API->>L: 返回 request_id
    L->>L: choice_callbacks[request_id] = cb
    ...
    DCS->>DISP: trigger DialogueChoiceSelectedEvent<br/>{request_id, option_index, choice_id}
    DISP-->>SEB: 桥接
    SEB->>L: emit "dialogue_choice_selected" payload<br/>{request_id, choice_index, choice_id}
    L->>L: cb = choice_callbacks[request_id]<br/>cb({id=..., index=..., zero_index=...})
    L->>L: choice_callbacks[request_id] = nil
```

**关键设计点**：

- **`request_id` 是关键**：Lua 用它把"原回调"与"返回事件"对上号。多个并发的 `tf.dialogue.choice` 调用各自有不同 request_id，互不干扰。
...
- **取消也走同一通道**：玩家按 Esc 关闭选项窗 → `DialogueChoiceSelectedEvent{cancelled=true}` → Lua 回调收到 `result.cancelled = true`。

### 6. Blueprint / EntityFactory 在脚本化实体上的角色

上一期 [part-23](../ref/OpenGL与迷你农场/23-蓝图与实体工厂.md) 讲过 Blueprint——把"角色 / 动物 / 作物"的组件组合从 C++ 类型挪到 JSON。**TinyFarmRPG 完整保留这套机制**，并把脚本化字段挂进 blueprint 字段约定。

举例：[`actor_blueprint.json`](../../assets/data/actor_blueprint.json) 里一个普通商人的最小定义可能这样：

```json
{
  "name": "actor.merchant",
  "components": {
    "NameComponent":      { "name": "Merchant" },
    "DialogueComponent":  { "dialogue_id": "default.merchant" }
  }
}
```

而在 Tiled 地图上，让某 actor object 引用这个 blueprint 时**可以再加 magic 属性**：

| Tiled 属性 | 起作用的层 |
| --- | --- |
| `blueprint_id="actor.merchant"` | EntityFactory 用此 id 挂 blueprint 默认组件 |
...
```mermaid
flowchart LR
    BP["Blueprint JSON<br/>核心组件组合"] --> EF["EntityFactory<br/>按 blueprint_id 挂默认组件"]
    TILED["Tiled 对象属性<br/>magic 字段"] --> EB["EntityBuilder<br/>解析并挂额外组件"]
    EF --> ENT["最终 Entity"]
    EB --> ENT
...
1. **Tiled 地图**：在 `home_exterior.tmj` 加一个 actor object，属性：
   - `blueprint_id="actor.farmer"`（如果 blueprint 已存在，否则需要 step 2）
   - `actor_id="npc.farmer"`
   - `scripted_interaction=true`
2. **Blueprint 配置**（可选，如果已有 blueprint 跳过）：[`actor_blueprint.json`](../../assets/data/actor_blueprint.json) 加一个 `actor.farmer` 定义。
...
| 3 | [`docs/tutorial/lua-content-authoring.md`](../../docs/tutorial/lua-content-authoring.md)（§4 Tiled 接入 + §5 配方） | 完整 Tiled 字段约定 + 8 个常用配方 |
| 4 | 上一期 [part-23 蓝图与实体工厂](../ref/OpenGL与迷你农场/23-蓝图与实体工厂.md) + [part-28 交互与对话](../ref/OpenGL与迷你农场/28-交互与对话.md) | 升级前的基础设施 |
...
4. **DialogueChoice 取消**：玩家按 Esc 关闭选项窗时，Lua 回调收到的 `result` 长什么样？写回调时该怎么区分"选了某项"和"取消"？
...
- **Tiled magic 属性**通过 `EntityBuilder` 解析成 component，最常用的 4 个：`actor_id`（脚本身份）/ `scripted_interaction`（C++ 让位）/ `script_event` + `script_once_key`（进入 payload 供脚本过滤）。
...
- **DialogueChoice** 用 `request_id` 把 Lua callback 与 C++ 选择结果对上号，Lua 回调收到 `{ id, index, zero_index, cancelled }`；**推荐用 `choice_id` 而非 index 判断选项**。
- **Blueprint + Tiled magic 属性**分工：Blueprint 提供默认组件组合，Tiled 字段挂额外组件（脚本身份 / 商店 / 招募）。
