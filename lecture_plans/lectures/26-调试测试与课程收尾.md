...
自测题 3。编译器能保证语法和类型，但保证不了这类**架构约定**：别在项目源码里写 C++ 异常关键字、别在 RML 之外硬编码 UI 文案、渲染分层别被绕过。这些规矩靠 code review 反复唠叨——而人会忘。
...
> **一个诚实的现状**：项目当前**没有提交 CI 配置**（仓库里没有 `.github/workflows`）。`engine_tests` / `game_tests` 已接入 `ctest`，普通 Debug 用 `gtest_discover_tests()`，ASan 用 `gtest_add_tests()`，这是**为门禁设计**的安全网；真正接到 GitHub Actions 这类 CI 还留待落地。所以本讲说"被门禁拦住"，当下的实义是"**提交前本地 `ctest` 全绿**"。
...
5. 项目再长两年，你赌哪个模块先腐烂？给出你的防御方案（用本讲学过的网里至少两件）。
...
- **坏输入不能写坏真相** = catalog、存档、配置读取都先验证再提交，失败保持默认或旧状态（L09/L21/L24/L26）。
