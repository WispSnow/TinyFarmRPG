...
围绕这个视角，本讲讲五件事：存档涵盖哪些状态、原子替换怎么做、迁移链怎么把旧档升到新版、新增组件时存档的接入 checklist、以及后台异步保存。还有一条贯穿的取舍——**项目未上线，开发期可以直接删档重来，但一旦 bump schema，仍要走迁移的流程**，这道边界本讲会说清楚。
...
```jsonc
{
  ...
  "party_state":  { "recruited_actor_ids": [...], "active_actor_ids": [...] },
...
一句话：**保存 = snapshot 当前地图 → 主线程 capture → 写 .tmp → rename 覆盖；加载 = 迁移到最新 → deserialize → apply + 发 sync 命令**。
...
std::filesystem::rename(tmp_path, file_path, ec);            // 原子地用 .tmp 顶替真档
```

这是自测题 1 的答案：**完整内容先写进 `slotX.json.tmp`，确认写成功后，再用 `rename` 一步把它覆盖到 `slotX.json`。** 文件系统层面 `rename` 是原子的——任何时刻 `slotX.json` 要么是**完整的旧档**、要么是**完整的新档**，永远不会是"写了一半"的状态。如果直接往原文件写，写到一半断电，真档就成了垃圾。代价只是多一个临时文件和一次 rename，换来的是"存档永不半写"的强保证。（rename 在已存在目标上失败的平台，代码会先 remove 再 rename 兜底。）
...
```cpp
const std::uint32_t schema_version = json.value(KEY_SCHEMA_VERSION, 0u);
if (schema_version == 0u)                  { out_error = "missing schema_version"; return false; }
if (schema_version > SAVE_SCHEMA_VERSION)  { out_error = "unsupported schema_version"; return false; } // 拒载未来版本

if (schema_version >= 2u && schema_version < 4u) { normalizeLatestFields(json); json[KEY] = 4u; }
if (json[KEY] == 4u) { migrateV4ToV5(json, out_error); }   // 每个小函数只管一步
if (json[KEY] == 5u) { migrateV5ToV6(json, out_error); }
if (json[KEY] == 6u) { migrateV6ToV7(json, out_error); }
...
| 资源 | 为什么读 |
| --- | --- |
| [`docs/game/save_and_flow.md`](../../docs/game/save_and_flow.md) | 存档全景：数据闭环图、snapshot 不变量、排错 checklist——本讲的权威底稿 |
| [`docs/tutorial/multi-thread/09-background-save-io.md`](../../docs/tutorial/multi-thread/09-background-save-io.md) | 后台异步保存的多线程细节（主线程抓取 / worker 写盘 / 命令队列回主线程） |
...
| 文件 | 看什么 |
| --- | --- |
| [`src/game/save/save_service.cpp`](../../src/game/save/save_service.cpp) | `writeSaveFile`(原子替换)、`capture`/`apply`、`saveToFileAsync`——**本讲主入口** |
| [`src/game/save/save_data.h`](../../src/game/save/save_data.h) / `.cpp` | `SAVE_SCHEMA_VERSION`、`json_keys`、`SaveData` 结构、`serialize`/`deserialize` |
| [`src/game/save/save_migrator.cpp`](../../src/game/save/save_migrator.cpp) | `migrateToLatest` 迁移链、`migrateVNToVN+1` 步骤、版本护栏 |
| [`src/game/save/save_slot_summary.h`](../../src/game/save/save_slot_summary.h) | `SlotSummary{day,timestamp}`、`tryReadSlotSummary` 轻量读取 |
...
- **接入 checklist**：SaveData 字段+键 → serialize/deserialize → capture → apply（含发 sync 命令）→ (破坏性则 bump+迁移) → (地图实体则 snapshot 覆盖)。
