...
`previewBuy` 是**纯只读**的——它把买入从头校验一遍（玩家有效？有钱包背包？数量合法？店里卖吗？买得起吗？背包放得下吗？），把结果填进 `ShopBuyPreview` 返回，**一个字节都不写**：

```cpp
preview.total_price = unit_price * quantity;
...
ShopBuyResult ShopTransactionService::commitBuy(...) const {
    ...
    const auto mutation = inventory_domain_service_.addItem(player, item_id, ...);  // 先加物品
    // ...异常则返回失败，金币未动...
    mutable_wallet->gold_ -= preview.total_price;                                   // 再扣钱
    return /* 成功 result */;
}
```
...
**(b) 先加物品、再扣钱（item-first）。** 顺序不是随便排的：先 `addItem`，若返回异常（理论上 preview 已保证有空间，但防御性检查）就返回失败、**金币一分没扣**。反过来若先扣钱再加物品、加物品却失败，玩家就"钱没了、东西没拿到"。Sell 方向对称：先 `removeItem` 再 `gold +=`，物品没扣成功就不会凭空加钱。
...
flowchart TD
    subgraph BUY["previewBuy 阶梯"]
        B1["玩家有效 + 有钱包/背包"] --> B2["数量 > 0 且不超 stack_limit"]
        B2 --> B3["shop_id 存在"] --> B4["item 存在"]
        B4 --> B5["该店有这条 buy_entry<br/>否则 ItemNotSoldHere"]
        B5 --> B6["金币够<br/>否则 InsufficientGold"]
        B6 --> B7["副本模拟放得下<br/>否则 InventoryFull"]
    end
...
失败原因是一个统一的 `ShopTradeFailureReason` 枚举（11 个值），UI 据此显示不同提示文本。这种"用枚举表达失败原因、而不是用 bool / 异常"的做法，让领域服务的每条失败路径都可被测试单独断言（对照 shop_transaction_service_test 的一排 `...FailsWhen...`）。
...
| 4 | [`tests/game/shop_transaction_service_test.cpp`](../../tests/game/shop_transaction_service_test.cpp) | 用 `...FailsWhenStateChangesAfterPreview` 等用例名反推不变量 |
...
| 顺序 | 文件 | 你会看到什么 |
| :---: | --- | --- |
| 1 | [`src/game/domain/shop_transaction_service.cpp`](../../src/game/domain/shop_transaction_service.cpp) | **本讲核心**——preview 只读校验 + commit 重新校验后原子写入，item-first 顺序 |
...
3. **item-first 顺序**：`commitBuy` 为什么是"先 addItem 再扣 gold"而不是反过来？举一个反过来会坑玩家的场景。
...
- **preview / commit 两段式**是 L10 领域服务模子的"公开化"版本：`preview` 纯只读、供 UI 实时显示；`commit` **内部重新 preview** 后才原子写入，堵死 TOCTOU。`canCommit()` 把关，item-first 顺序避免"钱没了东西没到"。
