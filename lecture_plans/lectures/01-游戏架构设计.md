...
上一讲我们建立了课程的全局地图。这一讲，我们正式进入项目内部：**站在新项目的入口（`GameScene::init()`）面前，看 TinyFarmRPG 如何把一个 4 万行的 TinyFarm 底座扩展成一个能继续生长的 JRPG 工程**。

如果你回想上一期 TinyFarm 的 `GameScene::init()`——里面手动 new 了 28 个 system，依赖顺序靠注释和经验维护。本期把这件事翻新了：catalog、service、system 的"创建"被拆到一组工厂里，`GameScene` 只剩下"按下启动键"的工作。
...
4. 新增一个跨场景共享的 service（例如"天气系统"）需要动哪几个文件？
...
flowchart TD
    ...
    RSF --> SVC["GameRuntimeServices<br/>持有 catalog + domain service + world + script"]
    SF --> BUN["GameSystemBundle<br/>持有 28+ ECS systems"]
...
回到 TinyFarm 的 `GameScene::init()`——你会看到 28 个 `make_unique<XxxSystem>(...)` 排成长长一列，依赖顺序靠注释和工程师记忆维护。**功能再多一倍，这个文件就要爆炸了**。
...
| `SystemFactory` | `runtime/system_factory.*` | 先补齐缺失的 domain service，再创建 28+ ECS system |
...
- **新增门槛低**：往 `GameRuntimeServices` 加一个字段、往 `SystemFactory::assemble` 里 `make_unique` 一行——就上车了。
...
sequenceDiagram
    ...
    RF->>RF: 初始化 Localization / AudioCue
    RF->>RF: 加载 GameTime / UserSettings
...
- **ScriptHost 最后**：脚本里 `tf.quest.status(...)`、`tf.shop.open(...)`、`tf.map.travel(...)` 之类的 API 全都依赖前面的 catalog 与 service 已经就位。

> **观察小练习**：打开 [`runtime_service_factory.cpp`](../../src/game/runtime/runtime_service_factory.cpp)，看 `assemble()` 主函数（约 60 行），数一数它有几次 `return false`——这些都是装配失败时的"硬停点"，对应上面流水线的依赖前置。
...
`GameScene` 持有 `GameRuntimeServices`（所有权）。但 system 与 UI 控件并不直接持有它——它们通过 `entt::registry::ctx()` 找到自己需要的服务。
...
- **避免"到处传指针"**：上一期 TinyFarm 在 part-26 中"手动给每个 System 传 Context 引用"——TinyFarmRPG 系统变多后，这种做法的参数列表会失控。`registry.ctx()` 让"谁需要谁取"成为自然模式。
...
| 3 | [`src/game/runtime/runtime_service_factory.cpp`](../../src/game/runtime/runtime_service_factory.cpp) | 装配流水线的"主函数"`assemble()`，约 80 行，看 `return false` 的位置就能读懂依赖顺序 |
| 4 | [`src/game/runtime/system_factory.cpp`](../../src/game/runtime/system_factory.cpp) | 先 ensure domain service，再 `make_unique` system 的模板 |
...
2. **ServiceLookup 解决了哪个具体痛点？** 在上一期 TinyFarm 里，UI 想拿到 `ItemCatalog` 是怎么做的？为什么 TinyFarmRPG 把它换成了"registry.ctx() + helper 函数"模式？
3. **顺序错位实验**：如果把 `RuntimeServiceFactory::assemble` 里 `initMapManager()` 与 `initWorldState()` 的位置对调，编译能通过吗？运行时会在哪一行炸？
...
3. **使用路径**：选一处使用点（推荐 [`ui/equipment_tab_content.cpp`](../../src/game/ui/equipment_tab_content.cpp)），观察它如何调用 `localization_->tr(key)` 把 i18n key 翻译成显示文本。
