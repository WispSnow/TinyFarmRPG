...
flowchart TD
    ...
    TIN --> TIS["QuestTurnInService::turnIn<br/>preflight → 写奖励 → completeQuest"]
...
// 1c. 关键：在背包槽位的【副本】上模拟放入全部奖励
std::vector<ItemStack> simulated_slots = inventory->slots_;   // ← 拷贝一份
for (const auto& reward_item : quest.rewards_.items_) {
    if (!simulateAdd(simulated_slots, ..., reward_item.count_, stack_limit)) {
        return makeFailureResult(QuestTurnInStatus::InventoryFull, kInventoryFullMessage);
    }
}
```

注意 `simulated_slots` 是 `inventory->slots_` 的**一份拷贝**。`simulateAdd` 在拷贝上把**全部**奖励物品都试着放进去，只要有任何一件放不下，就返回 `InventoryFull` 失败——**此时真实背包一个字节都没动**。

**第 2 段——原子写入**（preflight 全过了才到这里）：

```cpp
if (wallet && quest.rewards_.gold_ > 0) wallet->gold_ += quest.rewards_.gold_;       // 写钱包
for (const auto& reward_item : quest.rewards_.items_) {
    inventory_domain_service_.addItem(player, reward_item.item_id_hash_, ...);        // 写背包（经另一个领域服务）
}
quest_log_ops::completeQuest(quest_log, quest.id_);     // active → completed
quest_log_ops::eraseQuestProgress(quest_log, quest.id_); // 清进度
```

**为什么先模拟、再写入？** 因为奖励是"一笔交易"——50 金币 + 2 瓶药水必须**整笔成功或整笔不发**。如果不先 preflight，直接边发边检查，可能出现"金币加了、药水放不下"的半成品状态，玩家任务还没交付却已经拿了钱。先在副本上验证"整笔放得下"，是把原子性挡在写入之前最省事的办法。
...
| `RewardPreflightUsesSingleAccumulatedInventoryCopy` | preflight 用**同一份**累加副本（多件奖励一起算容量） |
...
> **回到自测题 3**：背包满时，preflight 第 1c 步用副本模拟就会拦下，返回 `InventoryFull`，**真实背包、钱包、任务状态全部不动**。奖励绝不会"半发一半"。
>
> **一个诚实的边界**：文档也写明，如果 preflight 通过后写入阶段仍出现异常的部分写入，当前实现只打 `warn log`、**不回滚**。对未上线的教学项目，preflight 已经挡住了绝大多数失败路径，回滚的复杂度暂不引入——这是有意识的取舍，不是疏漏。
...
| **领域** | `quest_turn_in_service_test`、`quest_battle_progress_resolver_test` | preflight 原子性、击败计数规则 | **不依赖 UI / 场景**，纯逻辑，跑得快、覆盖失败路径 |
...
| 4 | [`tests/game/quest_turn_in_service_test.cpp`](../../tests/game/quest_turn_in_service_test.cpp) | 用测试名反推领域服务必须守住的不变量 |
| 5 | [L02 领域服务与命令/事件边界](02-领域服务与命令事件边界.md) | 回看那个"命令 / 服务 / 事件"三角，本讲是它的第一个完整实例 |
...
| 顺序 | 文件 | 你会看到什么 |
| :---: | --- | --- |
| 1 | [`src/game/domain/quest_turn_in_service.cpp`](../../src/game/domain/quest_turn_in_service.cpp) | **领域服务样板**——preflight（副本模拟）→ 原子写入 → 状态迁移的完整三段 |
...
| 5 | [`src/game/domain/quest_battle_progress_resolver.cpp`](../../src/game/domain/quest_battle_progress_resolver.cpp) | 战斗 Victory → 击败计数推进进度（铺垫 L20） |
| 6 | [`src/game/scene/quest_offer_scene.cpp`](../../src/game/scene/quest_offer_scene.cpp) | 接取确认弹窗——Lua `tf.quest.offer` 推、玩家确认后回 `AcceptQuestCommand` |
| 7 | [`src/game/system/quest_interaction_system.cpp`](../../src/game/system/quest_interaction_system.cpp) | C++ fallback 状态机 + scripted 早退 |
...
3. **preflight 原子性**：玩家背包只剩 1 格空位，任务奖励是 2 种不同物品各 1 个。`turnIn` 会发生什么？金币（也是奖励的一部分）会先加上吗？哪个测试用例钉死了这个行为？
...
- **`QuestTurnInService` 是领域服务样板**：NotReady 校验 → preflight（在背包槽位**副本**上 `simulateAdd` 全部奖励）→ 原子写入（钱包 + 背包 + 状态迁移）。背包满则整笔拒绝，绝不半发，一排 `..._test` 用例钉死不变量。
