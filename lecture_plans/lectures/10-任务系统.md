...
上节课把"静态真相"打通了——catalog 里躺着 enemy、item、quest 的规则数据。但数据本身不会动。这节课我们让它**跑起来**：实现一条完整的 JRPG 任务闭环——

> 找 NPC **接任务** → 出门打怪，系统**自动计数** → 回 NPC **交付** → **奖励写回**背包和钱包。

这条线看着简单，却是整个项目的一个缩影。它第一次把 [领域服务概览](02-领域服务与命令事件边界.md) 留下的悬念——**"为什么写入操作要集中到领域服务"**——用一个能跑、能测、能调试的真实系统讲透。这节课的主角是 `QuestTurnInService`：你会看到一个领域服务到底长什么样、它的 **preflight → 原子写入 → 反馈** 三段式如何保证"要么全成功、要么不留痕迹"。看懂这一个，后面商店、装备的所有服务都是同一个模子。
...
## 先看再讲：把 quests.json 和 NPC 脚本并排看
...
**关键观察**：这个分工和 [Lua 内容层总览](06-Lua内容层总览.md) 那条边界严丝合缝——
...
> **先动手**：打开 ImGui 的 **QuestDebugPanel**，点 `Accept` → `Fill`（一键填满进度）→ `Turn In`，观察背包多出 2 个 potion、金币 +50。再点 `Clear` 重来。这条链路你能在面板里反复跑，这节课就是把它背后的每一跳讲清楚。
...
一句话记住：**Lua 发请求（command）→ 领域服务校验并原子写入 → 事件（event）通告 UI 与脚本**。这正是 [领域服务概览](02-领域服务与命令事件边界.md) 画的那个"命令 / 服务 / 事件"三角，这节课第一次落到真实代码上。
...
| | 持有者 | 内容 | 进存档吗 | 生命周期 |
| --- | --- | --- | --- | --- |
| **静态目录** | `QuestCatalog` | 任务定义：objective / reward / 标题 | ❌ 不进（来自 JSON） | 启动加载，只读 |
| **运行时真相** | `QuestLogComponent`（挂在玩家实体上） | 接了哪些、完成了哪些、各 objective 进度 | ✅ 进存档 | 随玩家存活，可变 |
...
**为什么必须分开？** 这是 [数据目录与 RPG Catalog](09-数据目录与RPG-Catalog.md) "catalog 不进存档"原则的直接后果：
...
注意一个细节：`active_quests` / `completed_quests` 里存的是**字符串 id 而不是 hash**（对照上节课，这里特意选了可读性）。原因正是"要写进存档、要能在调试面板里一眼看懂"——存档和日志是字符串的地盘。
...
> 选这种 schema 是为了**跨版本、跨重排的稳定性**——id 是内容契约，下标是排列顺序。前者你能保证不变，后者随时会变。

### 3. QuestTurnInService：领域服务的样板，逐段拆

这是本节课的核心。打开 [`quest_turn_in_service.cpp` 的 `turnIn`](../../src/game/domain/quest_turn_in_service.cpp)，它就是 [领域服务概览](02-领域服务与命令事件边界.md) 那句"preflight → 原子写入 → 反馈"的标准实现。逐段看：
...
**第 3 段——背包奖励批量原子写入**。物品奖励不再逐个 `addItem()`，而是交给 `InventoryDomainService::addItemsAtomically`：
...
- **靠 `source_enemy_id` 计数，不从名字反推**：`final_units` 里每个死掉的敌人单位带一个 `source_enemy_id`（指回 catalog 的 `enemy.slime`）。缺这个字段就**显式跳过**，绝不靠显示名猜——又是"稳定 id 优先"（呼应 [脚本事件桥](08-脚本事件桥与Tiled接入.md) 和 [数据目录](09-数据目录与RPG-Catalog.md)）。
...
这个 resolver 在后续战斗结算课程中会被 `GameScene` 调用，和奖励反馈合并成一条通知。这节课先建立"进度来自战斗 Victory"的因果即可。
...
并且 `DialogueSystem` 检测到 `QuestGiverComponent` 时会主动跳过，避免一次按键被两个系统同时处理。而 `scripted_interaction=true`（[脚本事件桥](08-脚本事件桥与Tiled接入.md)）会让 `QuestInteractionSystem` 自己也早退，把舞台让给 Lua。
...
| 层级 | 测试文件 | 测什么 | 为什么在这一层 |
| --- | --- | --- | --- |
| **数据** | `quest_catalog_test` | 加载、查找、`validateReferences` | 断引用要 fail-fast（[数据目录](09-数据目录与RPG-Catalog.md)） |
...
**优先级直觉**：新增一个领域服务，**先写 domain test**——它最便宜、最能覆盖"背包满 / 缺组件 / 重复操作"这些失败路径，而这些恰恰是最容易出 bug 的地方。scene smoke 是补充，不是替代。
...
| 顺序 | 文件 / 章节 | 关注点 |
| :---: | --- | --- |
| 1 | [`docs/gameplay/quest-system.md`](../../docs/gameplay/quest-system.md) | **本节课核心阅读材料**——分层图、progress key 规则、turnIn 执行顺序、交互优先级、扩展指南 |
...
| 6 | [领域服务概览](02-领域服务与命令事件边界.md) | 回看那个"命令 / 服务 / 事件"三角，本节课是它的第一个完整实例 |
...
| 6 | [`src/game/domain/quest_battle_progress_resolver.cpp`](../../src/game/domain/quest_battle_progress_resolver.cpp) | 战斗 Victory → 击败计数推进进度（铺垫后续战斗课程） |
...
- 想加一个**"采集 3 个素材"**的 objective 行不行？查一下 [`quest_data.h`](../../src/game/data/quest_data.h) 的 `QuestObjectiveKind`——目前**只有** `DefeatEnemyCount`。"采集计数"是一种**新规则**，需要在 C++ 里加一个新的 `QuestObjectiveKind` 枚举值 + 一个对应的 resolver（类比 `QuestBattleProgressResolver`），这**不是** Lua 能编排出来的。想一想：为什么"打怪计数"算规则真相（C++）、而"打完怪说什么话"算内容编排（Lua）？这正是 [Lua 内容层总览](06-Lua内容层总览.md) 那条边界的具体落点。
...
- **`QuestTurnInService` 是领域服务样板**：NotReady / 缺组件校验 → `next_quest_log` 副本准备 → `InventoryDomainService::addItemsAtomically()` 批量提交 reward item → 钱包和 quest log 最终提交。背包满或物品 id 异常都会整笔拒绝，绝不半发，一排测试用例钉死不变量。
...
任务这个"领域服务样板"立住了。下节课把**同一个模子**套到另一个核心玩法上。**[商店系统](11-商店系统.md)** 会讲 `ShopCatalog` 的"买入按店隔离、卖出全局共享"、`ShopTransactionService` 的 `previewBuy/commitBuy` 两段式（又是一个 preflight → 原子写入！），以及脚本商人如何按时间和任务状态切换 `shop_id`。你会发现交易和任务交付，本质是同一套思路。下节课见。
