...
这些测试构造的全是裸 `BattleUnit` 向量，**没有一行 `entt::registry`、没有一个 `Scene`**。本讲就讲清楚：被它们验证的那套规则，是怎么写出来的。
...
这就是自测题 2 的完整答案：**改 HP 的是战斗内的 `BattleUnit` 副本；回写的是 `writeBackBattleRuntimeStats`；时机是战斗结束事件；对上号靠 `source_actor_id`；写回后靠 `PartyRuntimeStatsChanged` 通知外层刷新。** 敌人没有 `source_actor_id`，所以它们的死活不写回任何持久数据——打完就没了。
...
| 文件 | 看什么 |
| --- | --- |
| [`src/game/battle/turn_core.h`](../../src/game/battle/turn_core.h) / [`.cpp`](../../src/game/battle/turn_core.cpp) | 速度排序、`advanceTurn` 死亡跳过、`evaluateOutcome` 判胜负、`forceOutcome` 逃跑与强制终局保持——**本讲主入口** |
...
下一讲 **L17 战斗动作解析**：把 Attack / Skill / Item / Guard / Escape 统一建模，讲透 `BattleActionResolver` 与 `BattleFormulaEvaluator` 的分工、普通攻击的 `damage` 为什么按实际扣血而不是理论伤害汇报、技能/物品/状态的目录驱动、战斗物品用的是运行时副本（结束才写回真实背包）、状态回合数如何递减，以及未来持续伤害最自然的扩展点。下讲见。
