...
下一讲 **L17 战斗动作解析**：把 Attack / Skill / Item / Guard / Escape 统一建模，讲透 `BattleActionResolver` 与 `BattleFormulaEvaluator` 的分工、普通攻击的 `damage` 为什么按实际扣血而不是理论伤害汇报、技能/物品/状态的目录驱动、战斗物品用的是运行时副本（结束才写回真实背包）、以及持续伤害状态在回合里何时结算。下讲见。
