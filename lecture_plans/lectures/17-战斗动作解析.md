...
[上节课](16-回合制战斗领域核心.md)把战斗骨架讲清楚了，但留了个黑盒——`BattleSession::submitAction` 里那句 `resolver_.resolve(action, turn_core, runtime_state)`。一次"用火球术打最左边那只史莱姆"，到底怎么从一个轻飘飘的 `BattleAction`（只带了"我要放 skill.fire_1，目标是它"）变成实打实的"扣 4 点 MP、过一次命中判定、按公式算出伤害、写进对方 HP、可能再附个状态、最后推进回合"？

这节课就拆这条结算链。核心是**两层分工**：`BattleActionResolver` 负责**编排规则**（校验、目标、消耗、状态、防御、逃跑），`BattleFormulaEvaluator` 负责**算数值**（把一个 Lua 公式字符串 + 攻防双方 → 一个整数）。围绕它你还会再看到几条贯穿全课的主线：**catalog 驱动**（技能/物品/状态全是数据）、**运行时副本**（战斗里动的库存和状态都不碰探索态真相）、以及 **"校验在前、写入在后、未命中也消耗"** 的结算纪律。
...
5. JSON 里的 `damage.variance / damage.critical` 当前是不是已经参与随机结算？如果没有，它们现阶段扮演什么角色？

---

## 先看再讲：测试名就是规则规格书

和[上节课](16-回合制战斗领域核心.md)一样，最好的观察入口是测试——[`tests/game/battle/battle_action_resolver_test.cpp`](../../tests/game/battle/battle_action_resolver_test.cpp) 里 21 个 case，几乎**每个 case 名就是一条规则**：
...
| `SkillMissConsumesMpAdvancesTurnAndSkipsEffects` | 技能 miss：扣 MP、推进回合、不造成伤害、不附加状态 |
...
这 21 个 case 全部用裸 `BattleUnit` 数组构造，**不开 BattleScene**。这节课讲的就是：被它们逐条钉死的这套规则，resolver 是怎么实现的。
...
flowchart TD
    ...
    SW -->|Attack| ATK["攻击目标校验<br/>单体敌人"]
    SW -->|Skill| TGT["collectTargets(scope)<br/>Scope → BattleUnit* 列表"]
    SW -->|Item| ITM["item_catalog 查表 → battle_use? → 检查 runtime item_stocks"]

    ATK --> AFORM["BattleFormulaEvaluator.evaluate<br/>普攻公式 a.atk"]
    AFORM --> ADMG["防御减半<br/>写目标 HP"]
    ADMG --> FIN["turn_core.refresh() → Ongoing 才 advanceTurn()"]

    TGT --> COST["扣 MP<br/>success_rate 命中判定"]
    COST -->|miss| MISS["miss=true<br/>advanceTurn<br/>不应用伤害/状态"]
    COST -->|hit| FORM["BattleFormulaEvaluator.evaluate<br/>Lua: a=施法者 b=目标 → int"]
...
这就是问题 3 的答案：**Resolver = 规则编排（校验、目标、消耗、状态、防御、逃跑），Evaluator = 纯算术。** 为什么算伤害要单拆一个类？文档说得直接：
...
这三道闸门是[上节课](16-回合制战斗领域核心.md)"唯一入口"在 resolver 内部的延续——**无论谁提交什么动作，都先确认"战斗还在打、确实轮到你、你还活着"**。`EndTurn / Guard / Escape` 不需要目标：前两者直接交回合，`Escape` 投骰子决定 `forceOutcome(Escaped)` 还是推进回合（呼应[上节课](16-回合制战斗领域核心.md)说的"逃跑走 forceOutcome 而非 evaluateOutcome"）。
...
这里还要顺手划一条边界：`DamageFormulaData` 里确实有 `variance` 和 `critical` 字段，`BattleActionResult` 里也有 `critical`，但**当前 resolver 没有做随机伤害浮动，也没有做暴击判定**。这些字段现阶段是"数据已解析 / 表现层可承接"的预留口：`BattleFormulaEvaluator` 只拿 `type + formula` 计算基础值，`BattleActionResolver` 也不会把 `damage.critical` 转成一次随机暴击。换句话说，如果你在 JSON 里把 `critical: true`，现在不会让伤害突然翻倍；后续若实现暴击，应该在 resolver 的命中后、写 HP 前补随机判定，并把结果写进 `BattleActionResult::critical`，让日志、HP 条和 popup 这些已有表现入口自然接上。
...
`runtime_state.item_stocks` 是[探索↔战斗的过渡](15-探索与战斗的过渡.md)入口处 `collectPlayerItemStocks` 拷进来、由 `BattleSessionOptions` 注入的副本。战斗全程只动它；打完，剩余库存随 `BattleEndedEvent::remaining_item_stocks` 带回探索态，由 `GameScene` 做差额写回（后续战斗结算课程细讲）。

这就是问题 2 的完整答案：**用药水扣的是副本；逃跑成功也照样保留这次扣减（副本已经少了一瓶）；真实背包要等战斗结束、按 `remaining_item_stocks` 算差额才更新。** 这条"战斗内动副本、结束按差额经领域服务集中写回"的纪律，和[任务系统](10-任务系统.md)任务交付、[装备成长课](13-装备成长与休息.md)装备事务、[上节课](16-回合制战斗领域核心.md)属性快照是同一个哲学——**探索态的真相只在结算阶段、以统一入口被改写**。
...
- **MP 在命中判定之前扣**。技能分支先扣 MP，再投命中骰；没打中（miss）仍然消耗 MP 并推进回合：
  ...
  ```
  这是问题 4 的答案：**miss 照样扣 MP、照样过回合**——把"挥空也是一次行动"建模进去，避免"反复试探零成本"。
  `SkillMissConsumesMpAdvancesTurnAndSkipsEffects` 这个回归测试专门锁住了另一半边界：miss 时不会套用主伤害，也不会执行 `add_state` 之类附加效果。
...
- **状态的回合计数在 round hook 递减**。技能用 `AddState` 加状态（几率门控，时长取 `state.min_turns_`），写进 `runtime_state` 而**不进 BattleUnit 快照**。它的递减发生在**轮次边界**——`BattleSession` 构造时给 `TurnCore` 装的 round hook 里：
  ...
  **诚实交代（这点和很多 JRPG 直觉不同）**：当前实现里状态**只是"限时命名标记"**——它有 `priority/min_turns/max_turns/traits`，会在 round hook 里按轮递减、在状态 HUD 上按 priority 排序显示，但**并没有"中毒每回合扣血"这类持续伤害（DoT）**。`state.poison` 的 `traits` 目前是空的，`traits_` 字段在战斗层根本没被消费。所以问题"持续伤害何时结算"的准确回答是：**目前没有 DoT；状态计数在 round hook（轮次边界）统一递减，而那里正是将来要加 DoT 时最自然的挂载点**（文档"扩展点"一节也把"扩展回合开始/结束 hook"列为状态系统的下一步）。把"现状"和"扩展点"分清楚，比硬讲一个还不存在的机制更有价值。
...
| 资源 | 为什么读 |
| --- | --- |
| [`docs/game/battle-internals.md`](../../docs/game/battle-internals.md)（七 Resolver / 五 submitAction trace / 十三 扩展点） | 实现者视角的规则真相与扩展指南；与[上节课](16-回合制战斗领域核心.md)的 `turn-based-battle.md` 交叉阅读 |
| [回合制战斗领域核心](16-回合制战斗领域核心.md) | `submitAction` 是本课 `resolve` 的上游；refresh/advance 的语义在那节课 |
...
| [`tests/game/battle/battle_action_resolver_test.cpp`](../../tests/game/battle/battle_action_resolver_test.cpp) | 21 个 case = resolver 行为规格书；动手练习的素材 |
...
| 文件 | 看什么 |
| --- | --- |
| [`src/game/battle/battle_action_resolver.cpp`](../../src/game/battle/battle_action_resolver.cpp) | `resolve` 前置校验 + 六种 type 分发；MP/库存消耗顺序；`applySkillEffects`/`applyBattleItemEffects`——**本节课主入口** |
...
5. `skills.json` 里的 `damage.variance / damage.critical` 现在会不会改变 resolver 的伤害结果？如果未来要实现暴击，你会把随机判定插在结算链的哪个位置？
...
- **运行时副本**：道具扣 `runtime_state.item_stocks` 副本、不碰探索态背包；剩余库存随 `BattleEndedEvent` 带回，后续课程按差额经 `InventoryDomainService` 写回，容量不足用 accepted/rejected 反馈。
- **结算纪律**：MP 先于命中扣（miss 也消耗，且不应用伤害/附加状态）、防御减半保底 1、状态在 round hook 按轮递减；当前**无 DoT**，`variance/critical` 也只是解析/表现预留，round hook 和命中后写入前分别是它们未来最自然的扩展点。

---

到这里，"一个 `BattleAction` 怎么被结算"已经透明了。但 `BattleAction` 本身是**谁造出来的**？

下节课把"玩家点菜单选技能/目标"和"敌人自动决策"统一看作**同一个抽象——`BattleAction` 的生产者**。玩家侧讲 `FlowState`/`MenuState` 双层状态机、菜单导航与 cursor memory、为什么不用 RmlUi 原生方向键导航；AI 侧讲 `BattleAiPlanner` 怎么按 rating 选技、按 scope 选目标、怎么用固定 seed 做确定性回归测试。两者最终都吐出一个 `BattleAction`，汇进这节课这条解算管线。下节课见。
