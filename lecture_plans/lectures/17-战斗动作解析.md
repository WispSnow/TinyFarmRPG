...
和 L16 一样，最好的观察入口是测试——[`tests/game/battle/battle_action_resolver_test.cpp`](../../tests/game/battle/battle_action_resolver_test.cpp) 里 21 个 case，几乎**每个 case 名就是一条规则**：
...
这 21 个 case 全部用裸 `BattleUnit` 数组构造，**不开 BattleScene**。本讲讲的就是：被它们逐条钉死的这套规则，resolver 是怎么实现的。
...
这就是自测题 2 的完整答案：**用药水扣的是副本；逃跑成功也照样保留这次扣减（副本已经少了一瓶）；真实背包要等战斗结束、按 `remaining_item_stocks` 算差额才更新。** 这条"战斗内动副本、结束按差额经领域服务集中写回"的纪律，和 L10 任务交付、L13 装备、L16 属性快照是同一个哲学——**探索态的真相只在结算阶段、以统一入口被改写**。
...
| [`tests/game/battle/battle_action_resolver_test.cpp`](../../tests/game/battle/battle_action_resolver_test.cpp) | 21 个 case = resolver 行为规格书；最小练习的素材 |
...
- **运行时副本**：道具扣 `runtime_state.item_stocks` 副本、不碰探索态背包；剩余库存随 `BattleEndedEvent` 带回，L20 按差额经 `InventoryDomainService` 写回，容量不足用 accepted/rejected 反馈。
