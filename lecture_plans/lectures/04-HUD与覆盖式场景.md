# 第4课 HUD 与覆盖式 UI 场景的生命周期

上节课我们把 RmlUi 接入到了项目里。这节课紧接着回答一个实际问题：**RmlUi 文档不会自己出现在屏幕上**——它必须被某个 C++ 对象 `load()` 出来、`unload()` 回去。那么，"长在屏幕上的所有 UI"分别归谁管？

打开 TinyFarmRPG 玩一会儿，你会看到两种截然不同的 UI：

- **常驻 HUD**：左下角快捷栏、右上角时钟、底部对话气泡、屏幕中央的提示文字——它们随 `GameScene` 入场创建，不会因为按 ESC 或打开背包而销毁重建。
- **覆盖式弹出**：按 ESC 出来的暂停菜单、按 I 出来的背包、走到商人面前打开的商店——它们压在 `GameScene` 上方，通常会冻结底层世界逻辑。

这两种 UI 用了**两套完全不同的机制**：HUD 是 `GameScene` 内部组合的多份独立 RmlUi 文档；覆盖式弹出是 `SceneManager` 栈上的独立 `Scene`。这节课讲清楚它们的边界、什么时候用哪种，以及它们怎么协作。
...
2. 打开背包菜单时，`GameScene` 的 `update` 是被暂停还是继续？为什么？
3. `DialogueChoice`（对话选项）为什么是一个独立 Scene 而不是常驻 HUD？
...
跑起 TinyFarmRPG 进入主世界，然后按下面顺序操作，注意观察 HUD 的"在"与"不在"：

1. 主世界里，快捷栏、时钟、提示条都在屏幕边缘显示。这是 HUD 的正常状态。
2. 按 `Esc` 打开暂停菜单——**HUD 还在**，暂停菜单半透明叠在上面。
3. 按 `I` 打开背包菜单——**HUD 依然在**。
4. 找商人打开商店——**底层 HUD 被隐藏了**，商店界面独占视觉焦点。
5. 触发一场战斗——**HUD 整套消失**，只剩战斗界面。

这就是这节课要解释的核心现象：**有的覆盖式场景保留底层 HUD，有的隐藏**。背后的决策机制只有一个一行接口——`uiCoverage()`。
...
> 回顾 [RmlUi 接入](03-RmlUi接入.md) 中 `owner_scene_id` 的概念：它不是"单文档槽位"而是"归属分组标签"。HUD 这种"一个 Scene 同时挂多份文档"的形态，正是 `owner_scene_id` 设计的核心用途。
...
| [`floating_notice.rml`](../../ui/rmlui/hud/floating_notice.rml) | `FloatingNoticeView` ×2 | 世界锚点浮动提示（"已拾取"、"任务进度+1"），同时挂 2 份避免冲突 |
...
**这些控件全部在 `GameScene::initUI()` 里一次性创建**，它们之间没有从属关系——`HotbarUI` 不知道 `TimeClockHud` 的存在。彼此独立的好处是：哪个出了 bug 单独排查，加新 HUD 只是再 `make_unique` 一个，不牵动其他控件。
...
**每个覆盖式 Scene 都是一个完整的 `engine::scene::Scene` 子类**——有自己的 `init` / `update` / `clean`、有自己的 `RmlDocumentController`，退出时通过 `requestPopScene()` 让 `SceneManager` 把自己弹掉。它们也继承了 Scene 自带的 `registry_`；需要读写底层玩法状态时，会显式拿到 `GameScene` 注入的 registry / service 非拥有引用，而不是假装自己就是主世界。
...
| Scene | 策略 | 为什么 |
| --- | --- | --- |
| `PauseMenuScene` | `Overlay`（默认） | 暂停菜单只是"叠加一层"，玩家应该能看到暂停时的世界画面和 HUD |
| `InventoryMenuScene` | `Overlay`（默认） | 玩家打开背包时，仍能瞥见时钟、对话条等 |
| `SaveSlotSelectScene` / `RestDialogScene` | `Overlay`（默认） | 保存与休息确认体量较小，保留底层 UI 帮助玩家保持上下文 |
| `ShopMenuScene` | `HideUnderlyingSceneUi` | 商店是全屏交易界面，需要独占视觉焦点 |
| `BattleScene` | `HideUnderlyingSceneUi` | 战斗界面必须独占画面，HUD（hotbar / clock / 浮动提示）全部隐藏 |
| `DialogueChoiceScene` | `HideUnderlyingSceneUi` | 选项弹窗本身有 modal 背板，HUD 露出来会视觉混乱 |
| `QuestOfferScene` / `RecruitOfferScene` | `HideUnderlyingSceneUi` | 全屏 modal 风格，聚焦在确认/拒绝的决策上 |
| `AppearanceCustomizeScene` | `HideUnderlyingSceneUi` | 换装界面要看角色预览，HUD 是干扰 |
...
**`RmlUiRuntime::setVisibleSceneOwners()` 接收这个列表后，按 `owner_scene_id` 批量切换文档可见性**。HUD 不需要被销毁——只是对文档调用 `Show()` / `Hide()`。开战或打开商店后回到主世界，HUD 再次出现。**显隐成本**远低于"销毁文档 / 重建文档"。

### 5. 栈顶独占 update，底层"冻结"

`SceneManager` 对栈的调度规则很简单：
...
- 玩家按 ESC 打开 `PauseMenuScene`，`GameScene::update` / `fixedUpdate` **立即停止被调用**——农场里的作物不再生长、NPC 不再走动，`GameSceneUiController::update()` 里的时钟、tooltip、对话逐字表现也不再推进。
...
**这就是 Scene 栈相对于"显隐文档"的核心价值**——它**冻结**底层逻辑，而不仅仅是隐藏画面。如果只用显隐来控制背包，作物在玩家整理背包时照样生长，玩家回到世界发现天黑了——这显然不是期望的行为。

### 6. 显隐 vs Scene push/pop 的取舍

知道了两种机制，剩下就是判断"什么用哪种"。

| 维度 | HUD 显隐切换 | Scene push/pop |
| --- | --- | --- |
| **生命周期成本** | 极低（一次 Show/Hide） | 较高（init / clean、文档 load / unload、registry 隔离） |
...
- ✅ **Tooltip 用 HUD**。鼠标划过物品图标的瞬间就要显示，移开立刻消失——频率极高、世界不该停。
- ✅ **背包菜单用 Scene**。玩家打开后会停下来看半分钟、做多步操作（拖、换装、丢弃）——必须冻结世界、必须独占输入。
- ✅ **对话气泡用 HUD**。它是 gameplay 过程中的展示层，跟 hotbar、tooltip 一样属于主世界的一部分。
- ✅ **对话选项用 Scene**。选项是确认链、有 `:focus` 焦点管理、必须独占输入——所以独立成 `DialogueChoiceScene`。**这也是为什么对话气泡（展示）与对话选项（交互）分了两套机制**——一个在 HUD 层，一个在 Scene 层。

### 7. 覆盖式 Scene 怎么"写回"主世界

覆盖式 Scene 不应该在 UI 回调里散落修改主世界组件。当前项目的做法是：**需要读取或提交 gameplay 状态时，由 `GameScene` 显式注入底层 registry / domain service；真正写入仍收敛到 domain service 或事件处理器**。
...
- **`ShopMenuScene` 持有底层 `game_registry_` 的非拥有引用**，但买卖提交不在 UI 层手写组件变更，而是调用 `ShopTransactionService::commitBuy/commitSell()`。这和 [领域服务概览](02-领域服务与命令事件边界.md) 中的原则一致——写入必须走单点入口。
...
代码里确实有 `game::runtime::GameMode` 和 `SystemScheduler::profileStages(GameMode)`，`GameScene::fixedUpdate()` 也会把 `game_mode_` 传给 scheduler。但在当前 UI 相关的路径里，`BattleScene` / 菜单 push 并没有自动调用 `GameScene::setGameMode()`；`setGameMode()` 目前只是一个可用接口。

因此，这节课不要把"进入战斗时底层 scheduler 切到 Battle profile"当作已接通的事实。当前覆盖式 UI 的冻结主要来自 `SceneManager` 的"只 update 栈顶"规则；`BattleScene` 是独立场景，并通过 `uiCoverage()` 隐藏底层 HUD。

> `GameMode` 的完整模型留到后续 SystemScheduler 课程复核。届时再判断它是否应该和探索/战斗过渡建立更强的运行时联动。
...
| 6 | RmlUi 子教程 spritesheet 章节 | HUD 大量用九宫格背景；提前过一遍语法 |
...
6. **交易写回**：商店购买成功后，金币和快捷栏分别靠什么路径更新？为什么当前没有 `WalletChangedEvent`？
...
这节课的核心就一句话：**常驻的用 HUD 文档，会中断世界逻辑的用 Scene push/pop**。
...
- 栈顶 Scene 独占 `update` / `fixedUpdate` / 输入——底层 `GameScene` 在覆盖时被冻结。
- `prepareUi()` / `render()` 遍历整栈；底层 Scene 使用 `alpha=1.0f`，避免冻结期间重复插值。
- `uiCoverage()` 是一行接口的策略开关：`Overlay`（默认）保留底层 HUD，`HideUnderlyingSceneUi` 隐藏底层 HUD。
- 选择规则：**世界逻辑该停 / 状态机复杂 / 频率不高 → Scene；纯展示 / 频率高 / 不影响逻辑 → HUD**。
- 覆盖式 Scene 的主世界写回收敛到 domain service 或 dispatcher——不在 UI 回调里散写组件。
- `GameMode` 目前只是接口线索，不是已接通的调度机制，留待后续课程复核。

---

## 下节课预告

UI 形态讲完了。下节课 **[输入上下文与菜单导航](05-输入上下文与菜单导航.md)** 回到"玩家怎么操控这些 UI"——Gameplay / Menu / Dialogue / Battle 四种输入上下文如何切换？为什么菜单导航不直接依赖 RmlUi 原生方向键？输入 glyph 是怎么和键位重绑定联动的？这是 UI 阶段最后一个、也是最容易被忽视的工程话题。
