...
```mermaid
flowchart TD
    ACTOR["ActorData<br/>class_id + 等级范围"] --> RESOLVE["resolveActorStats(catalog, actor, level, loadout)"]
...
equipment_tab_content.cpp        ← 装备页预览
...
`equipItem` 要协调三方——把物品从**背包槽位**取出、放进 **actor 的 loadout**、把换下来的旧装备**塞回背包**——所以它的 preflight 校验链是（文档列得很清楚）：

```mermaid
flowchart TD
    A["目标 actor 在 PartyComponent 里?"] --> B["物品来自有效背包槽?"]
    B --> C["该 item 在 RpgCatalog 有装备数据?"]
    C --> D["槽位 / 职业限制 / actor 限制都匹配?"]
    D --> E["换下的旧装备能安全回背包?"]
    E --> F["全过 → 原子换装 + 发 EquipmentChanged"]
```

最后那条"旧装备能否回背包"尤其体现原子性——测试 `UnequipItemFailsWhenInventoryIsFullAndKeepsLoadout` 钉死了它：**卸装时若背包满了放不下，整个操作失败、loadout 保持不变**。绝不会出现"装备脱了、东西却凭空消失"的半成品状态。这和 L10 任务交付的 `simulateAdd` preflight、L11 商店的 `canCommit` 是同一个模子——**先确认整笔能成，再动手**。

换装成功后发 `EquipmentChanged` 事件，UI（角色面板、装备页）收到后**重新调 `resolveActorStats`** 刷新显示——回到知识点 2 的闭环。
...
又是 **preview / apply 两段式**（和 L11 商店 preview/commit 同构）：玩家在 `RestDialogScene` 里拖动"休息几小时"，UI 实时用 `previewActivePartyRecovery` 显示"每个队员能回多少"；点确认才 `applyActivePartyRecovery` 写回 `PartyRuntimeStatsComponent` 并发 `PartyRuntimeStatsChanged`。
...
| 4 | [L11 商店系统](11-商店系统.md) | 对照 preview/commit——休息、换装都是同一个事务模子 |
...
| 3 | [`src/game/domain/equipment_domain_service.h`](../../src/game/domain/equipment_domain_service.h) | 换装的 preflight 校验链与原子 swap |
...
- 给 `equip_wooden_sword` 的 `param_bonuses` 加一项 `mhp: 100`（HP 上限 +100）。装备后观察 Character 面板的 HP 上限变化。再思考：装备这件武器**不会**让当前 HP 立刻 +100（对照知识点 4——只有"升级"那条路径会 += delta，换装只是改变上限，`normalizeState` 会把当前 HP clamp 到新上限内但不主动填充）。验证一下：满血时装上是不是没回血、残血时装上当前 HP 是否不变只是上限变高？
...
- **`EquipmentDomainService` 是又一个 preflight → 原子写入**：换装要校验队伍归属、槽位/职业限制、旧装备能否回背包；背包满则整笔失败、loadout 不变。
- **`PartyRestService` 是 preview/apply 显式事务**（同构 L11 商店），不在 update 轮询——因为休息是离散事件不是持续状态。
