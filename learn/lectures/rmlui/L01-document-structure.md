...
gl_renderer.loadRmlUiDocument("assets/ui/rmlui/learn_hello.rml");
```
...
bool RmlUiBasicsScene::init() {
    ...
    // 加载 RML 文档 —— 核心调用
    if (!gl_renderer.loadRmlUiDocument("assets/ui/rmlui/learn_hello.rml")) {
...
打开 `assets/ui/rmlui/learn_hello.rml`，尝试：
...
打开 `assets/ui/rmlui/learn_hello.rcss`，尝试：
...
创建一个新文件 `assets/ui/rmlui/learn_profile.rml` 和 `learn_profile.rcss`：
