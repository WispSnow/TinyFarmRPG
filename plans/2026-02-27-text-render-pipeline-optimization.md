...
- [ ] 修改 `createAtlasPage` 使用 `GL_R8` 内部格式、`GL_RED` 外部格式，并设置 texture swizzle（RGBA → ONE,ONE,ONE,RED）
- [ ] 修改 `loadGlyph` 直接上传 FreeType alpha buffer（处理 pitch 后直接 `glTexSubImage2D` 用 `GL_RED`），去掉 RGBA 转换的临时 vector
- [ ] 修改 `fillDebugInfo` 中显存统计：`width * height * 4` → `width * height * 1`
- [ ] 验证中文/英文/特殊符号的渲染正确性

### 2. 复用 HarfBuzz Buffer
...
- [ ] 在 `TextRenderer` 构造时创建 `hb_buffer_t*`，析构时销毁
- [ ] `shapeLine` 开头调用 `hb_buffer_reset()` 代替 `hb_buffer_create()`
- [ ] 移除 `shapeLine` 末尾的 `hb_buffer_destroy()`

### 3. 消除 per-glyph GL 状态查询
...
- **涉及文件**:
  - `src/engine/resource/font_manager.cpp` — `loadGlyph()`
  - `src/engine/render/opengl/gl_helper.h` — 已有 `ScopedGLUnpackAlignment` RAII
- **预期收益**: 减少 per-glyph 的 2 次 GPU 同步查询
- **推荐方案**:
  - 纹理绑定：上传完成后直接 `glBindTexture(GL_TEXTURE_2D, 0)` 恢复，无需先查询旧值（字形加载的调用上下文中不依赖之前的纹理绑定）
  - Unpack 对齐：使用已有的 `ScopedGLUnpackAlignment` RAII（`gl_helper.h:31`）替换手动 get/set，**仍需保证 unpack alignment 正确恢复**，避免全局 GL 状态泄漏影响其他纹理上传
- [ ] 移除 `loadGlyph` 中的 `glGetIntegerv(GL_TEXTURE_BINDING_2D)` 查询，上传后 bind 回 0
- [ ] 将手动 `glGetIntegerv(GL_UNPACK_ALIGNMENT)` + `glPixelStorei` 替换为 `ScopedGLUnpackAlignment` RAII
