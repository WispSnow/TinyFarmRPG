...
- 在 `game_overlay.rml` 中为 prompt bar 增加一个显隐绑定，例如：
  - `show_prompt_bar`
...
每个目标 scene 在文档根或按钮容器上监听 `mouseover`：
...
| 测试用例 | 说明 |
|---------|------|
| `GameplayContextAllowsTogglePromptBar` | `toggle_prompt_bar` 仅在 `Gameplay` context 下可触发 |
| `MenuContextBlocksTogglePromptBar` | 菜单类 context 中按 `F1` 不会触发该动作 |
