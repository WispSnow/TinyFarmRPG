...
flowchart TD
    ...
    SAVE["SaveService<br/>加载后归一化"] --> DOM

    DOM["game::domain::*<br/>规则与原子写入"] --> COMP["Components<br/>InventoryComponent ..."]
    DOM --> EVT["dispatcher<br/>InventoryChangedEvent ..."]
...
- `ShopMenuScene` 玩家买东西
- `Lua tf.inventory.add` 通过 `ScriptGameApi` 发命令
- `QuestTurnInService` 自身要在交付任务时发物品奖励
- 调试面板要直接塞物品做测试

如果每个地方都自己写一遍"找到 InventoryComponent → 合并堆叠 → 触发空槽 → 失败回退 → 发事件"，规则会迅速漂移。领域服务通过几条硬约束消除这个风险：

1. **原子写入**：一次调用要么全部成功，要么全部失败回滚（典型例子是 `ShopTransactionService::commitBuy` 同时改背包和金币）。
2. **统一事件**：所有写入都从同一处发 `InventoryChangedEvent` / `EquipmentChangedEvent` 等，UI 只需要订阅一处。
...
| **静态工具类**（全部方法 `static`，无成员） | `actor_progression_service.h` | `ActorProgressionService` | 任何地方按需调用 |
...
> 这种分类不是历史包袱，而是**有意区分**：长期持有外部依赖的（registry/dispatcher/catalog 引用）做成类成员便于注入；纯算法（经验曲线、HP/MP 恢复公式、战斗后任务推进）做成 static 避免不必要的对象生命周期；只操作单个组件的小工具用 namespace 函数最轻量。

## 各服务职责速览

### InventoryDomainService — 背包写入

唯一允许直接改 `InventoryComponent` 槽位的入口。

- `ensureInventory(entity)`：实体没有 `InventoryComponent` 就按 catalog 默认尺寸创建。
- `addItem(entity, item_id, count, preferred_slot=-1)`：合并堆叠 / 占用空槽 / 全失败回退。
- `removeItem(entity, item_id, count, slot=-1)`：按槽或按总量扣减。
- 任何成功的写入都通过 dispatcher 发 `InventoryChangedEvent`（携带 `changed_slots` diff），UI 与存档据此刷新。
...
三个静态类不需要构造，直接调用静态方法即可。`QuestBattleProgressResolver` 在战斗结算时 `resolver{};` 临时构造。
...
3. **写入必须发事件**：任何成功的写入都要发对应的 `*ChangedEvent`，UI/Save 据此刷新。这是"绕过 service 直接改组件"会破坏的关键不变量。
4. **service 不读 UI / 不开 Scene**：domain 只写组件 + 发事件。打开"任务交付完成"弹窗这种是 scene 层根据事件决定的。
5. **catalog 只读**：service 持有的 catalog 引用一律 `const`（或承诺不写），catalog 是规则真相，运行时不变。
...
2. **决定形态**：需要持有 catalog/dispatcher/registry 引用就做成类（放进 `GameRuntimeServices`）；纯函数 / 仅算法做成 static 类；只操作一个 component 做成 namespace 函数。
...
5. **决定要不要发新事件**：如果有 UI 需要订阅（"钓到鱼时弹窗"），新增 `FishCaughtEvent` 在 `game/defs/events.h`；如果只是物品变化，复用 `InventoryChangedEvent`。
...
- ❌ **把 commit 写成"尽力而为"**：失败时一半成功一半失败的状态对玩家是致命体验。要么完整成功，要么完整回滚。
- ❌ **把 catalog 校验放在 service 之外**：例如在 system 里手动 if 一遍"这个槽位接受不接受该装备类型"。规则应该集中在 service 内部，外面只看返回结果。

## 推荐代码阅读路径

按下面顺序读，能在 1 小时内建立完整心智模型：

1. `src/game/domain/inventory_domain_service.{h,cpp}` — 看怎么用 `addItem` 的合并堆叠、空槽分配、回滚。
