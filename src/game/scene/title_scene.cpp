...
bool TitleScene::initUI() {
    ...
    if (!rml_layer || !rml_layer->getContext()) {
        spdlog::error("TitleScene: RmlUILayer 或 Context 不可用。");
        return false;
    }

    auto constructor = data_bridge_.create(rml_layer->getContext(), MODEL_NAME);
    if (!constructor) {
        spdlog::error("TitleScene: 创建 data model '{}' 失败。", MODEL_NAME);
        return false;
    }

    constructor.Bind("error_text", &error_text_);
    constructor.Bind("has_error", &has_error_);

    error_text_ = error_message_;
    has_error_ = !error_message_.empty();

    document_ = loadRmlDocument(DOCUMENT_PATH);
    if (!document_) {
        spdlog::error("TitleScene: 加载文档 '{}' 失败。", DOCUMENT_PATH);
        data_bridge_.destroy();
        return false;
    }
