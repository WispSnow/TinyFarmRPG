...
void UIStackLayout::setCrossAxisAlignment(Alignment alignment) {
    if (cross_alignment_ != alignment) {
...
void UIStackLayout::onLayout() {
    ...
    for (const auto& child : children_) {
        ...
        // 主轴长度统一使用最终布局尺寸，包含 stretch / layout override 的结果。
    ...
    const auto resolveCrossAxisOffset = [this](float content_cross_size,
                                               float child_cross_size) -> float {
        if (cross_alignment_ == Alignment::Center) {
            return (content_cross_size - child_cross_size) * 0.5f;
        }
        if (cross_alignment_ == Alignment::End) {
            return content_cross_size - child_cross_size;
        }
        return 0.0f;
    };
    ...
    for (const auto& [child, child_main_axis_length] : visible_children) {
        ...
        const glm::vec2 child_size = child->getLayoutSize();

        if (is_vertical) {
            new_pos.y = start_offset_y + current_pos;
            new_pos.x = start_offset_x + resolveCrossAxisOffset(content_size.x, child_size.x);

            current_pos += child_main_axis_length + spacing_;
        } else {
            new_pos.x = start_offset_x + current_pos;
            new_pos.y = start_offset_y + resolveCrossAxisOffset(content_size.y, child_size.y);
