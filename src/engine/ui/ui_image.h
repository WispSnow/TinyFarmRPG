...
public:
     ...
     * @param source_rect 要绘制的纹理部分。（若尺寸为0，则不渲染）
    ...
    const engine::utils::Rect& getSourceRect() const { return image_.getSourceRect(); }
    void setSourceRect(const engine::utils::Rect& source_rect) { image_.setSourceRect(source_rect); }
