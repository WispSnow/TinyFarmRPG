...
public:
    ...
    [[nodiscard]] entt::entity getEntityID();             ///< @brief 获取实体ID（返回）

    /// @brief 允许子类为 LevelLoader 直接创建的实体补充游戏侧信息（例如 MapId）。
    /// @note 仅在 LevelLoader 未通过 Builder 创建实体时调用（例如 imagelayer / tilelayer 容器）。
    virtual void decorateExternalEntity([[maybe_unused]] entt::entity entity) {}
...
protected:
    ...
    // 工具函数
    const nlohmann::json* findPropertyJson(std::string_view property_name, std::string_view type_name);
    std::optional<std::string> getStringProperty(std::string_view property_name);
    std::optional<int> getIntProperty(std::string_view property_name);
};
